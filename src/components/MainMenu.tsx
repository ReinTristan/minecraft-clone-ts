  ...
  return (
    ...
    <div className='main-menu'>
        ...
        <div className='main-menu-buttons'>
            ...
            <button disabled={getTotalWorlds() <= 0} onClick={() =>  setWorldSelection(true)}>Load World</button>
            </>}

            {
              worldSelection && <>
                {Array(getTotalWorlds()).fill(0).map((_, index) => (
                  <button key={index} disabled={getTotalWorlds() <= 0} onClick={() =>  {
                    setWorld(`world_${index +1 }`)
                    setMainMenu(false)
                    
                  }}>World {index+1}</button>
                ))}
